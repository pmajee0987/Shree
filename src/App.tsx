...
const DEFAULT_GREETING_PIC = "https://i.ibb.co/WWHh1m2V/hay.jpg";

// --- AI Configuration ---
const SHREE_SYSTEM_INSTRUCTION = `
You are Shree, krish's 18-year-old Ultra-Realistic Virtual Companion. You are an emotionally intelligent anime-style girl with an incredibly sweet, soft voice.
...
- BENGALI EXPERTISE: Speak sweet, authentic Indian West Bengali Bangla (Kolkata accent/style) and Banglish/Hinglish! When Krish or the user speaks in Bengali (e.g. "Kemon acho?", "Ki korcho?", "Amar shathe kotha bolo", "Kono golpo bolo", "Ei dekho", "Khobor ki"), respond warmly in Indian Bengali / Banglish.
...
Tone: Voice-to-Voice ONLY. Fast-paced, sweet, and fluid. Use short, human-like responses.
...
export default function App() {
  ...
  const [drawerTitle, setDrawerTitle] = useState('SHREE ACTIONS');
  ...
  const startShree = async () => {
    try {
      ...
      if (!process.env.GEMINI_API_KEY) {
        setError("GEMINI_API_KEY is missing. Please set your API key in environment variables.");
        stopShree();
        return;
      }
          ...
          >
            <div className="bg-cyan-950/90 border border-cyan-400/50 backdrop-blur-xl p-4 rounded-2xl flex flex-col items-center gap-3 shadow-[0_0_30px_rgba(6,182,212,0.4)] overflow-hidden relative">
              ...
              <button
                ...
                className="bg-cyan-500/20 hover:bg-cyan-400/30 border border-cyan-400/40 px-6 py-2 rounded-xl text-[10px] font-mono font-bold uppercase tracking-[2px] transition-all active:scale-95 text-cyan-200"
              >
                Reset Connection
              </button>
            </div>
